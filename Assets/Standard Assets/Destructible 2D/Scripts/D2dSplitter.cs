...
namespace Destructible2D
{
	...
	[CustomEditor(typeof(D2dSplitter))]
	public class D2dSplitter_Editor : D2dEditor<D2dSplitter>
	{
		protected override void OnInspector()
		{
			BeginError(Any(t => t.Feather < 0));
				DrawDefault("feather", "If your destructible sprite has soft edges then you should increase the feather distance to the pixel thickness of your soft edges.");
			EndError();
			DrawDefault("healThreshold", "Split pieces cannot be healed by default, this is because healing split pieces can often lead to unexpected behaviour and worse performance. If you really want to allow healing of split pieces, then this setting allows you to control how many pixels must remain in the split piece for it to be eligible for healing.\n\n-1 = Split pieces cannot be healed.\n0 = All split pieces can be healed.\n10 = Split pieces must have more than 10 AlphaCount to be healed.");
...
namespace Destructible2D
{
	...
	[HelpURL(D2dHelper.HelpUrlPrefix + "D2dSplitter")]
	[AddComponentMenu(D2dHelper.ComponentMenuPrefix + "Splitter")]
	public class D2dSplitter : MonoBehaviour
	{
		...
		/// <summary>Split pieces cannot be healed by default, this is because healing split pieces can often lead to unexpected behaviour and worse performance. If you really want to allow healing of split pieces, then this setting allows you to control how many pixels must remain in the split piece for it to be eligible for healing.
		/// -1 = Split pieces cannot be healed.
		/// 0 = All split pieces can be healed.
